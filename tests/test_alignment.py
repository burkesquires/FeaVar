...
class TestAlignmentHandler:
    ...
    def test_load_alignment_with_duplicate_ids_raises(self, tmp_path):
        """Duplicate IDs cannot safely be used as analysis or merge keys."""
        alignment_path = tmp_path / "duplicates.fasta"
        alignment_path.write_text(">SEQ001\nACTG\n>SEQ001\nACTG\n")

        with pytest.raises(AlignmentError, match="identifiers must be unique"):
            AlignmentHandler(alignment_path)
    ...
    def test_find_reference_ambiguous_partial_raises(self, sample_alignment_file):
        """Test that ambiguous partial matches raise an error."""
        handler = AlignmentHandler(sample_alignment_file)

        with pytest.raises(ReferenceNotFoundError, match="ambiguous|Ambiguous"):
            handler.find_reference("SEQ0")  # Matches SEQ001, SEQ002, SEQ003
...
class TestAlignmentHandlerVariantExtraction:
    ...
    def test_extract_variant_with_gaps(self, sample_alignment_file):
        ...
        handler = AlignmentHandler(sample_alignment_file)
        handler.get_reference("SEQ002")

        # SEQ002 starts with gaps
        variant = handler.extract_variant(
            handler.find_reference("SEQ002"), [1, 2, 3, 4, 5]
        )
