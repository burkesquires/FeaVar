...
from feavar.core import AnalysisResult, FeaVarAnalysis
...
class TestAnalysisResult:
    ...
    def test_get_variant_for_accession_prefers_exact_match(self):
        """Test exact match is preferred over partial matches."""
        variants_df = pd.DataFrame(
            {
                "accession": ["SEQ0012", "SEQ001"],
                "variant_type": ["DEF", "ABC"],
            }
        )
        ...
        assert result.get_variant_for_accession("SEQ001") == "ABC"
...
class TestFeaVarAnalysis:
    ...
    def test_run_analysis_summary_format(self, sample_alignment_file):
        ...
        # Delta naming is the default, so the reference receives VT-REF.
        vt_labels = result.summary_df["VT"].tolist()
        assert "VT-REF" in vt_labels
...
class TestFeaVarAnalysisMetadata:
    ...
    def test_merge_metadata_defaults_to_analysis_records(
        self, sample_alignment_file, tmp_path
    ):
        """Metadata-only rows require an explicit outer join."""
        metadata_path = tmp_path / "metadata.tsv"
        metadata_path.write_text("accession\thost\nSEQ001\thuman\nEXTRA\tavian\n")
        analysis = FeaVarAnalysis(
            alignment_path=str(sample_alignment_file),
            reference_id="SEQ001",
            positions="10-20",
        ...
        result = analysis.run()
        merged = analysis.merge_metadata(metadata_path)
        outer_merged = analysis.merge_metadata(metadata_path, how="outer")

        assert len(merged) == result.num_sequences
        assert "EXTRA" not in merged["accession"].values
        assert "EXTRA" in outer_merged["accession"].values
