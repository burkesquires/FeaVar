...
def generate_delta_name(
    ...
    # Find differences
    differences = []
    for i, (var_res, ref_res) in enumerate(zip(variant_sequence, reference_sequence)):
        if var_res != ref_res:
...
def compute_variant_names(
    variants_df,  # pd.DataFrame with 'variant_type' column
    ...
    unique_variants = variant_counts.index.tolist()
...
def get_reference_variant_sequence(
    ...
    if match.empty:
        ...
        match = variants_df[variants_df['accession'].str.contains(reference_accession, na=False)]
    
    if match.empty:
        logger.warning(f"Reference accession '{reference_accession}' not found in variants")
        return None
    
    return match.iloc[0]['variant_type']
