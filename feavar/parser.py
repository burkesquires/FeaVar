...
class PositionParser:
    ...
    def parse(self, raw_positions: str) -> list[int]:
        ...
        if not position_coordinates:
            raise PositionParseError(f"No valid positions found in: '{raw_positions}'")
    ...
    def _parse_range(self, range_str: str) -> list[int]:
        ...
        try:
            ...
            raise PositionParseError(f"Invalid numbers in range: '{range_str}'") from e
    ...
    def _parse_single(self, pos_str: str) -> int:
        ...
        try:
            ...
            raise PositionParseError(f"Invalid position value: '{pos_str}'") from e

        if pos <= 0:
            raise PositionParseError(f"Position must be positive (1-based): {pos}")

        return pos

    def validate_positions(self, positions: list[int], sequence_length: int) -> bool:
        """
