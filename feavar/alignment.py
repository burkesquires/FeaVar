...
def infer_format_from_extension(filepath: Union[str, Path]) -> Optional[str]:
    """
...
class AlignmentHandler:
    ...
    def __init__(
        ...
        if alignment_format is None or alignment_format.lower() == "auto":
            ...
            if inferred:
                ...
                logger.info(f"Inferred format '{inferred}' from extension '{self.alignment_path.suffix}'")
            else:
        ...
        self._alignment: Optional[MultipleSeqAlignment] = None
    ...
    def find_reference(self, identifier: str) -> Optional[SeqRecord]:
        """
        Find a sequence by identifier (partial match supported).
